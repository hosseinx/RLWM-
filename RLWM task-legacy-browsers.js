...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'دستورالعمل اصلی.png', 'path': 'دستورالعمل اصلی.png'},
    {'name': 'default.png', 'path': 'https://pavlovia.org/assets/default/default.png'},
    {'name': 'دستورالعمل 2.png', 'path': 'دستورالعمل 2.png'},
  ]
});
...
async function experimentInit() {
  ...
  instruction = new visual.ImageStim({
    win : psychoJS.window,
    name : 'instruction', units : 'norm', 
    image : 'دستورالعمل اصلی.png', mask : undefined,
    anchor : 'center',
    ori : 0.0, 
    pos : [0, 0], 
    draggable: false,
  ...
  main_instruction = new visual.ImageStim({
    win : psychoJS.window,
    name : 'main_instruction', units : 'norm', 
    image : 'دستورالعمل 2.png', mask : undefined,
    anchor : 'center',
    ori : 0.0, 
    pos : [0, 0], 
    draggable: false,
...
function initial_instructionRoutineBegin(snapshot) {
  return async function () {
    ...
    initial_instructionComponents.push(instruction);
...
function main_blocks_instructionRoutineEachFrame() {
  return async function () {
    ...
    if (start_main_blocks.status === PsychoJS.Status.STARTED) {
      let theseKeys = start_main_blocks.getKeys({keyList: [], waitRelease: false});
...
function Learning_blockRoutineEnd(snapshot) {
  return async function () {
    ...
    response.stop();
    // the Routine "Learning_block" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
